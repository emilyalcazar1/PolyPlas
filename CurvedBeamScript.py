...
boundary_value_problem = available_boundary_value_problems["CurvedBeam"]
boundary_value_problem.applied_displacement_magnitude = 0.5
...
design_variable_filter = polyfilter.get_design_variable_filter(
    ...
    precomputed_data,
    filter_radius=5.0, 
    use_relative_filter_radius=True, # actual_filter_radius = filter_radius * largest_element_edge_length
    filter_type="polynomial", # Options: pde, pde_with_boundary_penalty, polynomial, and identity
    axis_of_symmetry='y' # symmetry about the specified axis. Options: x, y, xy, none (only implemented for polynomial filter)
)
...
polyplas_object = polyplas.PolyPlas(
    ...
    directory_name="CurvedBeam_Results"
)
...
polyplas_object.run_top_opt_problem(MMA_state, maximum_optimization_iterations=250, convergence_tolerance=5.0e-4, check_grad=False)
