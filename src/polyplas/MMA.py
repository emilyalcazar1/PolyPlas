...
This file provides the functions mmasub, subsolv. Additionally, I added perform_design_update
which is a simpler interface to mmasub.
"""
...
def perform_design_update(mma_state, objective_value, objective_gradient, constraint_values,
                          constraint_gradients, move_limit, iteration_number):
    number_of_design_variables = objective_gradient.size
    number_of_constraints = constraint_values.size
    c = 1000.0 * np.ones((number_of_constraints, 1))
    d = np.ones((number_of_constraints, 1))
    a0 = 1.0
    a = np.zeros((number_of_constraints, 1))
    xmma, _, _, _, _, _, _, _, _, lower_asymptotes, upper_asymptotes = \
        mmasub(number_of_constraints, number_of_design_variables, iteration_number,
                mma_state.current_design_variables,
                mma_state.design_variable_lower_bounds,
                mma_state.design_variable_upper_bounds,
                mma_state.previous_design_variables,
                mma_state.previous_previous_design_variables,
                objective_value, objective_gradient,
                constraint_values, constraint_gradients,
                mma_state.lower_asymptotes, mma_state.upper_asymptotes,
                a0, a, c, d, move_limit)
    mma_state.lower_asymptotes[:, 0] = lower_asymptotes[:, 0]
    mma_state.upper_asymptotes[:, 0] = upper_asymptotes[:, 0]
    mma_state.transfer_history(xmma)
...
def mmasub(m,n,iter,xval,xmin,xmax,xold1,xold2,f0val,df0dx,fval,dfdx,low,upp,a0,a,c,d,move):
    ...
    if iter <= 2:
        low = xval - asyinit * xmax_minus_xmin
        upp = xval + asyinit * xmax_minus_xmin
    else:
        ...
        lowmin = xval - 10.0 * xmax_minus_xmin
        lowmax = xval - 0.01 * xmax_minus_xmin
        uppmin = xval + 0.01 * xmax_minus_xmin
        uppmax = xval + 10.0 * xmax_minus_xmin
    ...
    zzz2 = xval - move * xmax_minus_xmin
    ...
    xmami = np.maximum(xmax_minus_xmin, 1.0e-5)
    xmamiinv = eeen / xmami
    ux1 = upp - xval
    ux2 = ux1 * ux1
    xl1 = xval - low
    ...
    P *= ux2.ravel()
    Q *= xl2.ravel()
    # P = (diags(ux2.ravel(), 0).dot(P.T)).T
    # Q = (diags(xl2.ravel(), 0).dot(Q.T)).T
...
# Function for solving the subproblem
def subsolv(m,n,epsimin,low,upp,alfa,beta,p0,q0,P,Q,a0,a,b,c,d):
    ...
    while epsi > epsimin:
        ...
        while (residumax > 0.9*epsi) and (ittt < 200):
            ...
            qlam = q0+np.dot(Q.T,lam)
            gvec = np.dot(P,uxinv1)+np.dot(Q,xlinv1)
            # GG = (diags(uxinv2.ravel(),0).dot(P.T)).T-(diags(xlinv2.ravel(),0).dot(Q.T)).T
            GG = (uxinv2.ravel() * P) - (xlinv2.ravel() * Q)
            dpsidx = plam/ux2-qlam/xl2
            ...
            if m < n:
                ...
                # Alam = np.asarray(diags(diaglamyi.ravel(),0) \
                #     +(diags(diagxinv.ravel(),0).dot(GG.T).T).dot(GG.T))
                Alam = (diaglamyi.ravel() \
                     + (diagxinv.ravel() * GG).dot(GG.T))
                ...
                Axx = np.asarray(diags(diagx.ravel(),0) \
                    +(diags(diaglamyiinv.ravel(),0).dot(GG).T).dot(GG))
            ...
            residumax = np.amax(np.abs(residu))
            steg = 2*steg
